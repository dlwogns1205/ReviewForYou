from functions import lets_do_crawling, keyword_in_review, sss, DNN_func, return_review_data, result_of_selected_review_s_same_reviews
...
def product_detail(request, id):
    ...
    positive_comments = [c for c in all_comments if c.score >= 0.6]
    negative_comments = [c for c in all_comments if c.score <= 0.4]
    ...
    if request.method == 'GET': #'positive_comments':positive_comments, 'negative_comments':negative_comments,
        return render(request, 'home/modal.html', {'TOP_Products':T_Products, 'RECENT_Products':R_Products, 'click_Product':click_Product,
                                                    'click_Product_keyword' :click_Product_keyword, 'pos_review':pos_review[:30], 'neg_review':neg_review[:30] })
    if request.method == 'POST':
        keywords = request.POST.getlist("rGroup", '')
        if keywords == '':
            return render(request, 'home/modal.html',
                          {'error1': '1개 이상 선택하세요!','TOP_Products':T_Products, 'RECENT_Products':R_Products, 'click_Product':click_Product,
                           'click_Product_keyword' :click_Product_keyword, 'pos_review':pos_review[:30], 'neg_review':neg_review[:30]})
        ...
        top3_products = [m for m, v in sorted(result.items(), key=lambda x: x[1], reverse=True)[:3]]

        return render(request, 'home/modal.html',{'TOP_Products':T_Products, 'RECENT_Products':R_Products, 'click_Product':click_Product,
                                                    'click_Product_keyword' :click_Product_keyword, 'pos_review':pos_review[:30], 'neg_review':neg_review[:30], 'recom_model' : top3_products})
...
def review_detail(request,review_id):
    ...
    for sen in sentences:
        # ws, vs = click_review.morph.split() , click_review.xai_vale.split()
        ws, vs, rate = DNN_func(sen)
        line = []
    ...
    if request.method == 'GET':  # 'positive_comments':positive_comments, 'negative_comments':negative_comments,
        ...
        result_same_sentences, same_rate = result_of_selected_review_s_same_reviews(selected_review, review_data,
                                                                                    vocab_sorted)
        return render(request, 'home/detail.html', {'TOP_Products':T_Products, 'RECENT_Products':R_Products, 'click_Product':click_Product,'click_review':click_review,
                                                    'recom_model':top3_products, 'result':temp,'click_review_keywords':keyword_eval,
                                                    'similar_review': result_same_sentences, 'similar_rate':same_rate })
    ...
    if request.method == 'POST':
        ...
        for review in other_reivew:
            text.append([review.score, review.review])
        review_data, vocab_sorted = return_review_data(text)
        result_same_sentences, same_rate = result_of_selected_review_s_same_reviews(selected_review, review_data, vocab_sorted)
        return render(request, 'home/detail.html',
                      {'TOP_Products':T_Products, 'RECENT_Products':R_Products, 'click_Product': click_Product, 'click_review': click_review,
                       'recom_model': top3_products, 'result': temp, 'click_review_keywords': keyword_eval,
                       'similar_review': result_same_sentences, 'similar_rate':same_rate})
    ...
    if request.method =='POST':
        ...
        if '/' in url_src:
            ...
            if url_src == '':
                ...
                if '11st' in url_src:
                    site = 1
                    tem_data, pre_product_name, img_src, price, review_len, categories, result, keyword, keyword_ratio = lets_do_crawling(site,product_num)
                ...
                product = ProductModel.objects.create(product_url=url_src, product_name=product_name,
                                                     img_src=img_src, price=price, review_len=review_len,categories = categories, product_num=product_num, search_value=1)
                product_score_list = []
                for index, row in tem_data.iterrows():
                    ...
                    review_model.review = row['review']
                    tmp_score = round(float(xai_positive_negative[index]),3)
                    ...
                    review_model.save()
                product.pos_neg_rate = round(sum([1 for i in product_score_list if i >0.5]) * 100 / (index+1))
                product.total_value = round(sum(product_score_list)* 10 / (index+1) , 1)
                product.save()

                for word, sentence in result.items():
                    # all_comments = ReviewModel.objects.filter(product_id=product)
                    # all_cnt, cnt = 0, 0
                    # for cm in all_comments:
                    #     if word in cm.review:
                    #         all_cnt += 1
                    #         if cm.score > 0.5:
                    #             cnt += 1
            ...
            if search_cate == '':
                return render(request, 'home/main.html',
                              {'error01': 'URL 혹은 카테고리를 입력해주세요!', 'TOP_Products': T_Products, 'RECENT_Products': R_Products})
            else:
                same_cate_products = []
                flag=0
                ...
                if flag != 1:
                    return render(request, 'home/main.html',
                                  {'error02': '해당하는 카테고리가 없습니다ㅠ','TOP_Products': T_Products, 'RECENT_Products': R_Products})

                T_Products = [p for p,_,_ in sorted(same_cate_products, key=lambda x:x[1], reverse=True)]
                R_Products = [p for p,_,_ in sorted(same_cate_products, key=lambda x:x[2], reverse=True)]
                return render(request, 'home/main.html', {'TOP_Products': T_Products, 'RECENT_Products': R_Products})
...
def trend(request, id):
    ...
    return render(request, 'home/trend.html', {'TOP_Products':T_Products, 'RECENT_Products':R_Products,'click_product':click_categories, 'id':id} )
